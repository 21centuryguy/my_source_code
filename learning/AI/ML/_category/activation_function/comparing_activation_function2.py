"""
http://blog.naver.com/PostView.nhn?blogId=wideeyed&logNo=221017173808&parentCategoryNo=&categoryNo=49&viewDate=&isShowPopularPosts=true&from=search
[출처] [딥러닝] Activation Function(활성화 함수) 소개 및 비교|작성자 똑똑이
"""
...
def activation_function_group1():
...
def activation_function_group2():
...
def activation_function_group3():
...
def activation_function_group4():
...
def activation_function_group5():
...
if __name__ == '__main__':
    Thread(target = activation_function_group1).start()
    Thread(target = activation_function_group2).start()
    Thread(target = activation_function_group3).start()
    Thread(target = activation_function_group4).start()
    Thread(target = activation_function_group5).start()
