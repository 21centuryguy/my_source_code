        ...
        if len(args) == 1:
            ...
            if type(color) == type(""):
                ...
                try:
                    ...
                    raise Error
                    # print("bad color string: %s" % `color`)
                self._color = color
                return
            try:
                r, g, b = color
                ...
                raise Error
                # print("bad color sequence: %s" % `color`)
        else:
            try:
                r, g, b = args
            except:
                raise Error
                # print("bad color arguments: %s" % `args`)
        assert 0 <= r <= 1
        assert 0 <= g <= 1
        assert 0 <= b <= 1
        x = 255.0
    ...
    def goto(self, *args):
        if len(args) == 1:
            try:
                ...
                raise Error
                # "bad point argument: %s" % `args[0]`
        else:
            try:
                x, y = args
            except:
                raise Error
                # "bad coordinates: %s" % `args[0]`
        x0, y0 = self._origin
