...
def printing_n_saving_lists(new_film_links, new_film_titles):
    ...
    # title_links = list(zip(new_film_titles[0:10], new_film_links[0:10]))

    """title_links = dict(zip(new_film_titles[0:10], new_film_links[0:10]))
    for xxx in title_links:
        print(xxx)"""
    ...
    i = 1
    # title_links = dict(zip(new_film_titles[0:-1], new_film_links[0:-1]))
    title_links = dict(zip(new_film_titles, new_film_links))
    # print("<<<<<<<"+str(len(title_links)+">>>>>>>"))
    """print(len(title_links))
    for key, value in title_links.items():
        print("[[[[[[[  "+str(i)+"  ]]]]]]")
        print(key+" : https://en.wikipedia.org/"+value)
        i = i + 1"""
    ...
    title_links = list(zip(new_film_titles, new_film_links))
    print("\n[ DEBUG ] line  " + str(cf.f_lineno) + " ==> title_links[0:10] : ", title_links[0:10], "\n")

    BASE_PATH = abspath(dirname(__file__))
    file_full_path = BASE_PATH + "/title_links.txt"
    with open(file_full_path, 'w') as f:
        for title_links in title_links:
            for title_links in title_links:
                title_links = title_links.encode('ascii', 'ignore')
                title_links = title_links.decode('utf-8')
                f.write(title_links)
                f.write('\n')
    f.close()
    """
    ...
    """ Big NG
    print("\n[ DEBUG ] line  " + str(cf.f_lineno) + " ==> new_film_titles[0:10] : ", new_film_titles[0:10], "\n")

    print("\n[ DEBUG ] line  " + str(cf.f_lineno) + " ==> new_film_links[0:10] : ", new_film_links[0:10], "\n")

    BASE_PATH = abspath(dirname(__file__))
    file_full_path = BASE_PATH + "/title_links.txt"
    i = 0
    with open(file_full_path, 'w') as f:
        while i < len(new_film_links):
            new_film_titles = new_film_titles[i] #.encode('ascii', 'ignore').decode('utf-8')
            print(new_film_titles)
            new_film_links = new_film_links[i] #.encode('ascii', 'ignore').decode('utf-8')
            print(new_film_links)
            f.write(new_film_titles + " :  https://en.wikipedia.org/" + new_film_links)
            f.write('\n')
            i = i + 1
            print("\n[ DEBUG ] line  " + str(cf.f_lineno) + " ==> i : ", i, "\n")
    # f.close()
    """
    ...
    BASE_PATH = abspath(dirname(__file__))
    file_full_path = BASE_PATH + "/title.txt"
    i = 0
    with open(file_full_path, 'w') as f:
        for new_film_titles in new_film_titles:
            new_film_titles = new_film_titles.encode('ascii', 'ignore').decode('utf-8')
            print(new_film_titles)
            f.write(new_film_titles)
            f.write('\n')
            print("\n[ DEBUG ] line  " + str(cf.f_lineno) + " ==> i : ", i, "\n")
    f.close()
    ...
    i = 0
    with open(file_full_path, 'w') as f:
        for new_film_links in new_film_links:
            new_film_links = new_film_links # .encode('ascii', 'ignore').decode('utf-8')
            print(new_film_links)
            f.write("https://en.wikipedia.org/" + new_film_links)
            f.write('\n')
            print("\n[ DEBUG ] line  " + str(cf.f_lineno) + " ==> i : ", i, "\n")
    f.close()
    """
