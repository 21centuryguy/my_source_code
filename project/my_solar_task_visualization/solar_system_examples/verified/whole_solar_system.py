...
while t < seconds:
    ...
    for plan in range(len(planets)):
        ...
        planetsf[plan].pos = rotate(planetsf[plan].pos,angle=planets[plan].varAngularPosition(t,dt.s),axis=vector(0,0,1))
    # v = rotate(v,angle=moon.varAngularPosition(t,dt.s,m2=2),axis=(0,0,1))
    v = rotate(v,angle=moon.varAngularPosition(t,dt.s,m2=2),axis=vector(0,0,1))
    mo.pos = ea.pos + v
    t += dt.s