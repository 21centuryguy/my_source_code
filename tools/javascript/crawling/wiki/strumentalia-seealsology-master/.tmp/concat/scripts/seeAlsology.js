...
'use strict';
angular.module('wikiDiverApp').controller('AltCtrl', [
  '$scope',
  '$http',
  '$log',
  '$timeout',
  '$interval',
  '$window',
  function ($scope, $http, $log, $timeout, $interval, $window) {
    var languages = {
        en: {
          name: 'english',
          seeAlso: ['See also'],
    ...
    $scope.supportedLanguages = Object.keys(languages).map(function (l) {
    ...
    try {
      Object.keys(localStorage).forEach(function (k) {
        if (k.indexOf('seeAlsology-update-') !== 0)
          return;
        var p = k.replace('seeAlsology-update-', 'seeAlsology-'), u = p.replace('seeAlsology-', '');
        if (parseInt(localStorage.getItem(k)) < yest) {
          localStorage.removeItem(k);
          localStorage.removeItem(p);
        } else
          $scope.cacheLinks[u] = localStorage.getItem(p).split('|');
      });
    } catch (e) {
    }
    ...
    $scope.startCrawl = function () {
      ...
      // Draw sigma legend
      $('.sigma-legend').empty();
      $scope.colors.slice(0, $scope.depth + 3).forEach(function (c, i) {
        $('.sigma-legend').append('<span>' + '<div style="background-color: ' + c + '"></div>' + '&nbsp;&nbsp;' + (i ? 'level ' + (i - 2) : 'seeds') + '</span>');
      });
    ...
    $scope.validate = function () {
      ...
      var regex = /https?:\/\/([a-z]+)\.wikipedia\.org\/wiki\/.+/i, testUrl = null, listOfPages = $scope.query.split('\n'), lang = '', langs = [], errors = [];
      // check for integrity
      ...
      if (errors.length)
        ...
        checkParentsDepth();
        $scope.lang = langs[0];
        var curSW = $scope.stopWords.map(function (s) {
            return s.text;
          });
    ...
    $scope.$watchCollection('[getParents, depth]', function (newV, oldV) {
      if (newV === oldV || $scope.alert && $scope.alert.indexOf('collecting') !== 0)
        return;
      checkParentsDepth();
    });
    ...
    function downloadPageSeeAlsoLinks(pageLink, callback, updateResolved) {
      ...
      if ($scope.cacheLinks[pageLink]) {
        if ($scope.cacheLinks[pageLink][0] === '#NOT-FOUND#')
          ...
          callback(filterStopWords($scope.cacheLinks[pageLink]));  // or find the page's SeeAlso section from API
      } else
        $http.jsonp('http://' + $scope.lang + '.wikipedia.org/w/api.php?action=parse&page=' + pageLink + '&prop=sections&format=json&redirects&callback=JSON_CALLBACK').success(function (data) {
          if (!data.parse)
          ...
          sections.forEach(function (section, i) {
            ...
            $http.jsonp('http://' + $scope.lang + '.wikipedia.org/w/api.php?format=json&action=query&prop=revisions&titles=' + pageLink + '&rvprop=content&rvsection=' + section.index + '&redirects&callback=JSON_CALLBACK').success(function (linksData) {
              if (!linksData.query)
                return notFound(pageLink, updateResolved);
              // Collect links from the section content
              var o = linksData.query.pages[Object.keys(linksData.query.pages)[0]].revisions[0]['*'], linksRegex = /\[\[(.*?)\]\]/g, matches = linksRegex.exec(o), links = [];
              while (matches) {
    ...
    function addEdge(source, target, ind) {
      ...
      addNode(target, ind);
      var sourceId = source.toLowerCase(), targetId = target.toLowerCase(), edgeId = sourceId + '->' + targetId;
      if ($scope.edgesIndex[edgeId])
        return;
      $('#edges').append('<tr>' + '<td>' + discreetLink(source) + '</td>' + '<td>' + discreetLink(target) + '</td>' + '<td>' + ind + '</td>' + '</tr>');
    ...
    function getParents(page, ind, seed) {
      if ($scope.doneParents[page])
        return;
      $scope.doneParents[page] = true;
      // Get backlinks to the page from the API
      $http.jsonp('http://' + $scope.lang + '.wikipedia.org/w/api.php?action=query&bltitle=' + page + '&blnamespace=0&list=backlinks&blredirect&blfilterredir=nonredirects&bllimit=250&format=json&callback=JSON_CALLBACK').success(function (data) {
    ...
    function validateParentFromLinks(page, parentLink, ind, links) {
      var name = linkToTitle(page), check = name.toLowerCase();
      if (links.some(function (l) {
          return l.toLowerCase() === check;
        })) {
        $scope.edgesQueue.push({
    ...
    $scope.$watch($scope.working, function (n, o) {
      if (!n && n !== o)
        $timeout(function () {
    ...
    $scope.zoomSigma = function (positiveZoom) {
      ...
      sigma.misc.animation.camera(cam, { ratio: cam.ratio * (positiveZoom ? 1 / 1.5 : 1.5) }, { duration: 150 });
    };
    $scope.recenterSigma = function () {
      sigma.misc.animation.camera($scope.sigma.cameras[0], {
        x: 0,
        y: 0,
        angle: 0,
        ratio: 1
