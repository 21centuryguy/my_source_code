...
import sys
import shutil
import selenium.webdriver.chrome; target ="selenium.webdriver.chrome"
...
def help_target(target):
	...
	# x = dir(sklearn) # 1 번째
	x = dir(selenium.webdriver.chrome)
	...
	with open("result/result.txt","w+") as archi:
		t = sys.stdout
		sys.stdout = archi
		help(selenium.webdriver.chrome)
		sys.stdout = t
...
def import_target(target, package_list):
	...
	#---------------------------------------------
	#--- generate and return target_import_line_list
	target_import_line_list = []
	for verified_package in verified_package_list:
		target_import_line = 'import ' + verified_package + '; target ="' + verified_package + '"'
		target_import_line_list.append(target_import_line)

	print("="*100)
	print("target_import_line_list :", target_import_line_list)
	print("="*100)

	i = open("result/target_import_line.txt", mode="w+")
	i.write("")
	for target_import_line in target_import_line_list:
		i.write(target_import_line)
		i.write("\n")
	i.close()

	return target_import_line_list
