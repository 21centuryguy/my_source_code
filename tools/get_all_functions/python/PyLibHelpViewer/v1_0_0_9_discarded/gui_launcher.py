...
# - get builtin lib list
def get_builtin_lib_list(target_builtin_lib_path):

	listdir_result_object = os.listdir(target_builtin_lib_path)
	...
	items.append("="*40)
	items.append(target_builtin_lib_path)
	items.append("="*40)
...
# ------------
# --- read [ python env lib path ] and call func [ get_builtin_lib_list ]



if len(target_builtin_lib_path_list) == 0:

	def check_if_you_want_to_add_lib_path():

		def cancel_add_path():
			print("directory selection is canceled.")

		def add_path():
			target_builtin_lib_path_list = get_target_builtin_lib_path()
			for target_builtin_lib_path in target_builtin_lib_path_list:
				get_builtin_lib_list(target_builtin_lib_path)

		root = Tk()
		root.title("Add Python Library Path")
		frame = ttk.Frame(root, padding=8)
		frame.grid()

		lb = Label(frame, text="You can add a python lib path.", bg="mistyrose", font=("Helvetica 12" ))
		b1 = Button(frame, text="Cancel", command=lambda:[cancel_add_path(), root.iconify()])
		b2 = Button(frame, text="Add Path", command=lambda:[add_path(), root.iconify()])

		lb.grid(row=0)
		b1.grid(row=1, sticky=W)
		b2.grid(row=1, sticky=E)

	check_if_you_want_to_add_lib_path()
